...
class RemoveMerchItemController extends Controller {
	...
	protected function deleteInDatabaseAndReturn(Request $request) {
		...
		catch (ModelNotFoundException $ex) { return back()->with('error', "ERROR AL REMOVER EL ARTÍCULO: No hay un artículo de merchandising con nombre " . $request->input('nombre') . " en la base de datos."); }
		return back()->with('success','¡Artículo removido con ÉXITO!');
	}
